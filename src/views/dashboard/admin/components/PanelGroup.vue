...
export default {
  ...
  methods: {
    loadData(){
      ...
      _this.$store.dispatch("bi/getHomeCount").then(res => {
        _this.loading = false;
        if(res.length>0){
          var dataObject=res[0];
          _this.newCustomer=dataObject.NewCustomer;
          _this.collectionMoney=dataObject.CollectionMoney;
          _this.willPay=dataObject.WillPay;
        }
      }).catch(err => {
          _this.$message({
